            ...
            for future in as_completed(future_to_variant):
                ...
                try:
                    ...
                    if variant_update:
                        ...
                        variant_info = " - ".join(variant_info_parts)
                        logger.info(f"    ✅ Updated variant {variant_info}: {variant_change.change_description}")
                        logger.info(f"         📦 Product: {variant_change.product_title}")
                        logger.debug(f"         🏷️  Metafield: {self.namespace}.{self.metafield_key} = {variant_change.new_quantity}")
                    else:
